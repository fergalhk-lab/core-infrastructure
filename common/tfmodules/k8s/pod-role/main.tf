locals {
  ...
  is_wildcard_principal = var.namespace == "*" || var.service_account_name == "*"
  role_name = coalesce(
    var.role_name,
    local.is_wildcard_principal
...
data "aws_iam_policy_document" "assume_role" {
  statement {
    ...
    condition {
      # StringLike when any wildcard is present, StringEquals for exact match
      test     = local.is_wildcard_principal ? "StringLike" : "StringEquals"
